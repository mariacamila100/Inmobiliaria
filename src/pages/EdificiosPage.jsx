  ...
  return (
    <div className="bg-white/60 backdrop-blur-xl rounded-2xl shadow-sm overflow-hidden animate-fadeIn">
      ...
      <div className="p-6 flex flex-col md:flex-row justify-between items-center gap-4 bg-slate-100/70">
        <h3 className="font-bold text-slate-800 whitespace-nowrap text-lg">Copropiedades</h3>
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50/60 border-b border-slate-100">
            <tr className="text-[11px] text-slate-500 uppercase tracking-wide">
              <th className="px-8 py-4 text-left font-semibold">Nombre</th>
              <th className="px-8 py-4 text-left font-semibold">Ciudad</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {currentItems.map((edi) => (
              <tr key={edi.id} className="hover:bg-slate-50/50 transition duration-200">
                <td className="px-8 py-4 font-bold text-slate-700">{edi.nombre}</td>
